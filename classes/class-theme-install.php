					...
					foreach ( $themes as $theme ) {
						...
						<article class="cp-plugin-card<?php echo esc_attr( $vetted_article ); ?>" id="cp-plugin-id-<?php echo esc_attr( $slug ); ?>">
							<header class="cp-plugin-card-header">
								<h3>
								<?php
								echo esc_html( $theme['title']['rendered'] );
								echo wp_kses_post( $vetted );
								?>
								</h3>
								<div class="cp-plugin-author"><?php echo wp_kses( sprintf( __( 'By <b>%1$s</b>.', 'classicpress-directory-integration' ), $theme['meta']['developer_name'] ), array( 'b' => array() ) ); ?></div>
							</header>
