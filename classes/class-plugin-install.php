					...
					foreach ( $plugins as $plugin ) {
						...
						<article class="cp-plugin-card<?php echo esc_attr( $vetted_article ); ?>" id="cp-plugin-id-<?php echo esc_attr( $slug ); ?>">
							<header class="cp-plugin-card-header">
								<h3>
								<?php
								echo esc_html( $plugin['title']['rendered'] );
								echo wp_kses_post( $vetted );
								?>
								</h3>
								<div class="cp-plugin-author"><?php echo wp_kses( sprintf( __( 'By <b>%1$s</b>.', 'classicpress-directory-integration' ), $plugin['meta']['developer_name'] ), array( 'b' => array() ) ); ?></div>
							</header>
							...
							<footer class="cp-plugin-card-footer" data-content="<?php echo esc_attr( $content ); ?>">
								...
								<div class="cp-plugin-actions">
									...
									if ( ! array_key_exists( $slug, $local_cp_plugins ) ) {
										...
										) . '" class="button install-now">' . esc_html__( 'Install', 'classicpress-directory-integration' ) . '</a>';
									}
									...
									if ( array_key_exists( $slug, $local_cp_plugins ) && ! $local_cp_plugins[ $slug ]['Active'] ) {
										...
										) . '" class="button button-primary">' . esc_html__( 'Activate', 'classicpress-directory-integration' ) . '</a>';
									}
									?>
								</div>
							</footer>
				...
				<nav aria-label="<?php esc_attr_e( 'Plugin search results navigation', 'classicpress-directory-integration' ); ?>">
					<ul class="cp-plugins-pagination">
						...
						for ( $x = 1; $x <= $pages; $x++ ) {
							...
							$link         = '<a href="' . esc_url( add_query_arg( array( 'getpage' => $x ), remove_query_arg( array( 'getpage', 'searchingfor' ) ) ) ) . '">' . (int) $x . '</a>';
